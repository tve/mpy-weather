...
class BME680(BME680Data):
    ...
    def __init__(self, i2c_addr=I2C_ADDR_PRIMARY, i2c_device=None):
        ...
        # perform a whole conversion so we can set gas stuff
        self.convert()
    ...
    def convert(self):
        """Start conversion of temperature, humidity, pressure, and gas
        Returns milliseconds to wait
        """
        self.set_power_mode(FORCED_MODE)
        return 36 + (self.gas_settings.heatr_dur if self.gas_settings.heatr_dur != None else 100)

    def ready(self):
        """Returns true when a conversion has completed
        """
        status = self._get_regs(FIELD0_ADDR, 1)
        return (status & NEW_DATA_MSK) != 0

    def read_data(self):
        """Get sensor data.

        Returns a 4-tuple with temperature, humidity, pressure, and gas resistance
        """
        ...
        return self.data.temperature, self.data.humidity, self.data.pressure, self.data.gas_resistance
