...
class Anemo:
    ...
    def start(self, loop, gust_ms=3000):
        """
        Starts the measurements, including launching a background asyncio poller to measure the wind
        gust speed every gust_ms interval.
        ...
        """
        loop.create_task(self._gust_poller(gust_ms))
    ...
    def read(self):
        ...
        now = time.ticks_ms()
        count = self.ctr.value()
        if self.ws_at == None or time.ticks_diff(now, self.ws_at) < 30000:
            speed = 0
            gust = 0
        else:
...
class Vane:
    """
    Vane implements the measurement of wind direction using an analog wind vane that reports degrees
    of rotation (direction) as an analog voltage.
    """
    ...
    def __init__(self, pin, v_min, v_max, offset):
        ...
        if isinstance(pin, int):
            ...
            try:
                ...
                raise ValueError("pin does not have read_u16 method")
            self.pin = pin
    ...
    def _raw_read(self):
        # oversample 5x and use median value
        vals = sorted([self.pin.read_u16() for _ in range(5)])
        val = vals[len(vals)//2]
    ...
    def start(self, loop):
        ...
        loop.create_task(self._vane_poller())
